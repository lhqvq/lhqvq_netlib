...
EventLoop::EventLoop()
    : lock_(),
      quit_(false),
      loopThreadId_(current_thread::Tid()),
    ...
    if (detail::t_loopOfCurrentThread != nullptr) {
        LOG_FATAL << "Already exist a EventLoop in this thread.";
    }
    detail::t_loopOfCurrentThread = this;
...
EventLoop::~EventLoop() {
    ...
    ::close(eventfd_);
    detail::t_loopOfCurrentThread = nullptr;
}
...
void EventLoop::Loop() {
    ...
    while (!quit_) {
        ...
        poller_->Poll(detail::kPollTimeoutsMs, &activeChannels_);
        ...
        std::vector<Functor> functors; { // 加锁. 其他线程也会访问 pendingFunctors_.
...
EventLoop *EventLoop::CurrentThreadLoop() {
    return detail::t_loopOfCurrentThread;
}
